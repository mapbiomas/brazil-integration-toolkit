...
# Versioning
#
VERSION_INPUT_INTEGRATION   = '0-4-3-c5a-2'
VERSION_SILVICULTURA_FILTER = f'{VERSION_INPUT_INTEGRATION[0:3]}-4-sil-3'
VERSION_GENERAL_FILTER      = f'{VERSION_INPUT_INTEGRATION[0:3]}-5-gen-3'
VERSION_AGRICULTURE_FILTER  = f'{VERSION_INPUT_INTEGRATION[0:3]}-6-agr-3'
VERSION_PASTURE_FILTER      = f'{VERSION_INPUT_INTEGRATION[0:3]}-7-pas-3'
VERSION_ALERTAS_FILTER      = f'{VERSION_INPUT_INTEGRATION[0:3]}-8-ale-3'
VERSION_TRANSITIONS_FILTER  = f'{VERSION_INPUT_INTEGRATION[0:3]}-9-tra-3'

# Year range between 1985 and 2025
YEARS = list(range(1985, 2026))

# Asset paths
COLLECTION_ID_STR     = str(COLLECTION_ID).replace('.', '_')
ASSET_COLLECTION      = f'projects/mapbiomas-{REGION_NAME}/assets/LAND-COVER/COLLECTION-{COLLECTION_ID_STR}'
...
# ==============================
GENERAL_KERNEL3_YEARS = YEARS[1:-1][::-1]       # (1985-2025) 1986 a 2024 (reverso)
GENERAL_KERNEL4_YEARS = YEARS[2:-1][::-1]       # (1985-2025) 1987 a 2024 (reverso)
GENERAL_KERNEL3_LAST_YEAR = [YEARS[-2]]         # (1985-2025) 2024
...
ALERTAS_TARGET_CLASSES = [3, 4, 5, 6, 7, 49, 11, 50, 12, 13, 21]
...
TRANSITIONS_NATURAL_CLASSES = [3, 4, 5, 6, 7, 11, 12,  32, 49, 50]
...
def wait_until_tasks_finish(export_tasks=[], polling_interval=60):
    """
    Waits until all export tasks have finished, printing a summary line each poll cycle.
    """
    if not export_tasks:
    ...
    while True:
        statuses = [t.status() for t in export_tasks]
        ...
        print(
            ...
            f"Total={total}  COMPLETED={counts.get('COMPLETED', 0)}  "
            f"RUNNING={counts.get('RUNNING', 0)}  READY={counts.get('READY', 0)}  "
            f"FAILED={counts.get('FAILED', 0)}"
        )

        running = [s['description'] for s in statuses if s['state'] == 'RUNNING']
        if running:
        ...
        for s in statuses:
            ...
            if s['state'] == 'FAILED' and desc not in completed:
                err = s.get('error_message', 'No error message provided.')
                failed_tasks.append((desc, err))
                completed.add(desc)
                print(f"    {desc} → {err}")

        for s in statuses:
            if s['state'] in ('COMPLETED', 'CANCELLED', 'FAILED'):
                completed.add(s.get('description', ''))

        if len(completed) == total:
            elapsed_final = str(timedelta(seconds=int(time.time() - start_time)))
            print(f"\n✅ All tasks finished. Total time: {elapsed_final}")
            if failed_tasks:
                print(f"\n❌ {len(failed_tasks)} task(s) failed:")
                for desc, err in failed_tasks:
                    print(f"  {desc} → {err}")
            break
...
def apply_alertas_filter(classification_img):
    ...
    for year in ALERTAS_YEARS:
        ...
        for biome_key, biome_id in ALERTAS_BIOME_IDS.items():
            ...
            # Classe de saída para alertas permanentes (do dicionário ALERTAS_CLASS_OUTPUT)
            class_out_perm = ALERTAS_CLASS_OUTPUT[biome_key]
