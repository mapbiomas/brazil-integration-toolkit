...
# 1. Ancorar o caminho no local do ARQUIVO, não no terminal
# file_path será: .../integration_pipeline/1_brazil/script.py
file_path = Path(__file__).resolve()

# 2. Subir dois níveis para chegar em D:\MapBiomas_GIT\repositories
# Nível 1: /1_brazil/ -> /integration_pipeline/
# Nível 2: /integration_pipeline/ -> /repositories/
repo_root = file_path.parents[2] 

if str(repo_root) not in sys.path:
    sys.path.append(str(repo_root))

print(f"DEBUG: Root do repositório adicionada: {repo_root}")

# 3. Importação
try:
    from utils.region_utils import RegionUtils
    print("Sucesso: RegionUtils carregado.")
except ModuleNotFoundError as e:
    print(f"ERRO: Não encontrei a pasta 'utils' em: {repo_root}")
    print(f"Conteúdo de {repo_root}: {os.listdir(repo_root)}")
    sys.exit(1)
...
# Versioning
#
VERSION_INPUT_INTEGRATION   = '0-04-02-c21-1'
VERSION_SILVICULTURA_FILTER = '0-04-03-sil-1'
VERSION_GENERAL_FILTER      = '0-04-04-gen-1'
VERSION_AGRICULTURE_FILTER  = '0-04-05-agr-1'
VERSION_PASTURE_FILTER      = '0-04-06-pas-1'
VERSION_ALERTAS_FILTER      = '0-04-07-ale-1'
VERSION_TRANSITIONS_FILTER  = '0-04-08-tra-1'

# Year range between 2017 and 2025
YEARS = list(range(2017, 2026))

# Asset paths
ASSET_COLLECTION      = f'projects/mapbiomas-{REGION_NAME}/assets/LAND-COVER-10M/COLLECTION-{COLLECTION_ID}'
...
ASSET_BIOMES          = 'projects/mapbiomas-workspace/AUXILIAR/biome_2025_buf5k_30m'
ASSET_GRID            = 'projects/mapbiomas-workspace/AUXILIAR/cim-world-1-250000'
ASSET_ALERTAS         = 'projects/mapbiomas-brazil/assets/ANCILLARY/ALERTS/alerta_2026_06_10'
...
EXPORT_SCALE = 10  # in meters
EXPORT_MAX_PIXELS = 1e13
EXPORT_PYRAMIDING_POLICY = "MODE"

NORONHA_GEOMETRY = ee.Geometry.Polygon([[
...
SILVICULTURE_NATURAL_CLASSES = [3, 4, 5, 6, 49, 11, 12, 50, 13]

# classes que devem ser remapeadas para 2
SILVICULTURE_FARMING_CLASSES = [15, 18, 19, 39, 20, 40, 62, 41, 36, 46, 47, 48, 21,75]
...
# ==============================
GENERAL_KERNEL3_YEARS = YEARS[1:-1][::-1]       # (2017-2024) 2018 a 2023 (reverso)
GENERAL_KERNEL4_YEARS = YEARS[2:-1][::-1]       # (2017-2024) 2019 a 2023 (reverso)
GENERAL_KERNEL3_LAST_YEAR = [YEARS[-2]]         # (2017-2024) 2023
...
GENERAL_CLASSES_BIOME = {
    ...
    'mat': [3, 4, 11, 12, 13, 29, 49, 50, 24, 25, 15, 9, 19, 36, 23, 30, 46, 47, 48],
    'pam': [25, 9, 3, 50, 11, 29, 30, 24, 18, 23, 49, 12],
    'pan': [3, 15, 25, 30, 41],
    'bra': [],
}
...
GENERAL_EXCEPTIONS = {
    ...
    'bra': {
        'k3': [
            # remove 21 before pasture in a deforestation
            [[3, 21, 15], [3, 21, 15]],
            [[4, 21, 15], [4, 21, 15]],
            [[11, 21, 15], [11, 21, 15]],
            [[12, 21, 15], [12, 21, 15]],
            [[49, 21, 15], [49, 21, 15]],
            [[50, 21, 15], [50, 21, 15]],
            # add 1 year of 21 before any forest recovery
            [[15, 3, 3], [21, 3, 3]],
            [[24, 3, 3], [24, 21, 3]],
            [[30, 3, 3], [30, 21, 3]],
            [[39, 3, 3], [39, 21, 3]],
            ...
            [[41, 3, 3], [41, 21, 3]],
            [[46, 3, 3], [46, 21, 3]],
            # add 1 year of 21 before any savana recovery
            [[15, 4, 4], [21, 4, 4]],
            [[24, 4, 4], [24, 21, 4]],
            [[30, 4, 4], [30, 21, 4]],
            [[39, 4, 4], [39, 21, 4]],
            ...
            [[41, 4, 4], [41, 21, 4]],
            [[46, 4, 4], [46, 21, 4]],
            # add 1 year of 21 before any grassland recovery
            [[15, 12, 12], [21, 12, 12]],
            [[24, 12, 12], [24, 21, 12]],
            [[30, 12, 12], [39, 21, 12]],
            [[39, 12, 12], [39, 21, 12]],
        ...
        'k3_last': [],
        'k4': [
            # fix 21 before pasture
            [[3, 21, 21, 15], [3, 21, 15, 15]],
            # fix recovery between mining
            [[30, 3, 3, 30], [30, 30, 30, 30]],
            [[30, 4, 4, 30], [30, 30, 30, 30]],
            [[30, 11, 11, 30], [30, 30, 30, 30]],
            [[30, 12, 12, 30], [30, 30, 30, 30]],
            [[30, 15, 15, 30], [30, 30, 30, 30]],
            [[30, 21, 21, 30], [30, 30, 30, 30]],
            [[30, 29, 29, 30], [30, 30, 30, 30]],
            # fix forest plantation between forest
            [[3, 9, 9, 3], [3, 3, 3, 3]],
            # fix forest before agriculture or silviculture
            [[21, 3, 3, 9], [21, 9, 9, 9]],
...
TRANSITIONS_MIN_CONNECTED_PIXELS = 25
...
def export_by_grid(image, asset_output, task_description_prefix, version, description):
    ...
    for grid_name in GRID_NAMES:
        ...
        if grid_name == 'SB-25':
            grid = grid.merge(ee.FeatureCollection([ee.Feature(NORONHA_GEOMETRY)]))
...
def get_markdown_agriculture_filter():
    ...
    **Description:**
    Este dataset aplica um filtro de agricultura para suavizar inconsistências temporais nos mapas anuais de uso e cobertura da terra. O processo inclui:
    - Remapeamento das classes de agricultura em três grupos principais.
    - Aplicação de regras temporais para corrigir transições inconsistentes.
    - Preservação de transições legítimas, garantindo a integridade dos dados.
...
def get_markdown_pasture_filter():
    ...
    **Description:**
    Este dataset aplica um filtro de pastagem para suavizar inconsistências temporais nos mapas anuais de uso e cobertura da terra. O processo inclui:
    - Identificação de transições rápidas envolvendo a classe de pastagem (classe {PASTURE_CLASS_ID}).
    - Remoção de pequenos agrupamentos de pixels que representam ruído, com base em um tamanho mínimo definido.
    - Preservação de transições legítimas, garantindo a integridade dos dados.
...
def get_markdown_alertas_filter():
    ...
    **Description:**
    ...
    Este dataset aplica um filtro de alertas para identificar e remover inconsistências temporais nos mapas anuais de uso e cobertura da terra. O processo inclui:
    - Identificação de pixels com alertas de mudança (como desmatamento).
    - Aplicação de regras específicas para classificar ou corrigir pixels com alertas.
    - Preservação de dados legítimos, garantindo a integridade dos dados.
...
def get_markdown_transition_noise_filter():
    ...
    **Description:**
    ...
    Este dataset aplica um filtro de ruído de transição para suavizar inconsistências temporais nos mapas anuais de uso e cobertura da terra. O processo inclui:
    - Identificação de transições rápidas entre classes naturais e antrópicas.
    - Remoção de pequenos agrupamentos de pixels que representam ruído, com base em um tamanho mínimo definido.
    - Preservação de transições legítimas, garantindo a integridade dos dados.
    
    **Notas Metodológicas:**
    
    1. O filtro analisa sequências temporais para detectar mudanças abruptas que não são sustentadas ao longo do tempo.
    2. Pequenos agrupamentos de pixels (definidos pelo parâmetro de tamanho mínimo) são reclassificados para a classe dominante ao redor.
    3. O filtro é aplicado globalmente, mas pode ser ajustado para diferentes regiões conforme necessário.
    **Versão de Saída:** `{VERSION_TRANSITIONS_FILTER}`
    **Tamanho Mínimo de Agrupamento:** `{TRANSITIONS_MIN_CONNECTED_PIXELS}` pixels
    **Classes Naturais:** `{TRANSITIONS_NATURAL_CLASSES}`
    **Classes Antrópicas:** `{TRANSITIONS_ANTHROPIC_CLASSES}`
...
def apply_silvicultura_filter(image):
    ...
    def remap_classification(image):
        ...
        # Verificando as bandas existentes
        band_names1 = image.bandNames().getInfo()
        band_names2 = ee.ImageCollection(remapped_bands).toBands().bandNames().getInfo()
        print(f"silvicultura original_image encontradas ({len(band_names1)}): {band_names1}")
        print(f"silvicultura remapped_bands encontradas ({len(band_names2)}): {band_names2}")
...
def apply_general_filter(classification):
    ...
    def apply_rules_by_biome(biome_key):
        ...
        biome_ids = BIOME_IDS[biome_key]
        biome_mask = biomes.remap(biome_ids, [1] * len(biome_ids), 0)
    ...
    # 'bra' entra primeiro como base (cobre o país inteiro, sem smoothing por classe);
    # os biomas específicos entram depois e sobrepõem 'bra' onde tiverem pixel válido,
    # preservando as correções feitas com as classes/exceções de cada bioma.
    filtered_images = [apply_rules_by_biome('bra')]
    ...
    filtered = ee.ImageCollection.fromImages(filtered_images).mosaic()
